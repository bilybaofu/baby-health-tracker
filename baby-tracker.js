...
class BabyHealthTracker {
    constructor() {
        ...
        this.tempExtractedData = null; // 临时存储可编辑的识别数据
        this.init();
    }
        ...
        babyInfoInputs.forEach(id => {
            ...
            if (input) {
                input.addEventListener('change', () => {
                    this.saveBabyInfo();
                    this.updateCurrentStatus(); // 立即更新显示
                });
            }
        });
    ...
    // 修改：自动计算月龄 - 使用基本信息中的出生日期
    autoCalculateAge() {
        const birthDate = this.babyInfo.birthDate; // 使用保存的基本信息中的出生日期
    ...
    async handleImageUpload(file) {
        ...
        if (typeof Tesseract === 'undefined') {
            this.showMessage('🔧 OCR识别库未加载，请刷新页面重试', 'error');
            return;
        }
        ...
        try {
            ...
            this.showLoadingState('🔍 正在初始化OCR识别...');
    ...
    showOCRResult(ocrText) {
        ...
        resultDiv.innerHTML = `
            <div style="text-align: center; margin-bottom: 20px;">
                ...
                <p style="color: #666; font-size: 0.9em;">✨ 文字识别完成，接下来可以进行智能解析</p>
            </div>
            ...
            <div style="text-align: center; display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                <button onclick="babyTracker.parseOCRResultManually()" class="btn btn-primary cute-btn" style="background: linear-gradient(135deg, #4FACFE 0%, #00F2FE 100%); padding: 12px 25px; border-radius: 25px; font-weight: 600; box-shadow: 0 5px 15px rgba(79, 172, 254, 0.4);">
                    🤖 智能解析数据
                </button>
                <button onclick="babyTracker.hideOCRResult()" class="btn btn-secondary cute-btn" style="background: linear-gradient(135deg, #DDA0DD 0%, #DA70D6 100%); padding: 12px 25px; border-radius: 25px; font-weight: 600;">
                    ❌ 关闭结果
                </button>
            </div>
        `;
...

{
  "records": [
        ...
        parseDiv.innerHTML = `
            <div style="text-align: center; margin-bottom: 20px;">
                <h4 style="color: #FF1493; margin-bottom: 10px; font-size: 1.4em;">
                    🎯 智能识别结果 <span style="background: #FF69B4; color: white; padding: 5px 12px; border-radius: 15px; font-size: 0.8em;">${recordsCount}条记录</span>
                </h4>
            ...
            <div style="text-align: center; margin-top: 20px; display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                <button onclick="babyTracker.saveEditedRecords()" class="btn btn-success cute-btn" style="background: linear-gradient(135deg, #FF69B4 0%, #FF1493 100%); padding: 12px 25px; border-radius: 25px; font-weight: 600; box-shadow: 0 5px 15px rgba(255, 105, 180, 0.4);">
                    ✨ 保存并添加记录
                </button>
                <button onclick="babyTracker.hideOCRResult()" class="btn btn-secondary cute-btn" style="background: linear-gradient(135deg, #DDA0DD 0%, #DA70D6 100%); padding: 12px 25px; border-radius: 25px; font-weight: 600;">
                    ❌ 取消
    ...
    async performOCR(file) {
        try {
            console.log('开始OCR识别...文件大小:', file.size, '类型:', file.type);
                ...
                'chi_sim+eng',
                {
                    logger: m => {
                        ...
                        if (m.status === 'recognizing text') {
                            ...
                            this.showLoadingState(`🔍 正在识别文字... ${progress}%`);
                        } else if (m.status === 'loading tesseract core') {
                            this.showLoadingState('🔧 正在加载OCR核心...');
                        } else if (m.status === 'initializing tesseract') {
                            this.showLoadingState('⚙️ 正在初始化OCR...');
                        } else if (m.status === 'loading language traineddata') {
                            this.showLoadingState('🇨🇳 正在加载中文语言包...');
                        } else if (m.status === 'initializing api') {
                            this.showLoadingState('🤖 正在初始化识别引擎...');
                        }
                    }
                }
            );
            ...
            console.log('OCR识别完成');
            console.log('识别置信度:', data.confidence);
            console.log('识别文本长度:', data.text.length);
    ...
    updatePercentiles() {
        ...
        const gender = this.babyInfo.gender || 'male'; // 使用保存的性别信息
        const percentiles = {
    ...
    updateRecordsTable() {
        ...
        sortedRecords.forEach((record, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button onclick="babyTracker.deleteRecord(${this.checkRecords.indexOf(record)})" class="btn btn-danger" style="padding: 8px 15px; font-size: 0.8em; border-radius: 15px; background: linear-gradient(135deg, #FF6B6B 0%, #FF5252 100%);">
                        🗑️ 删除
                    </button>
                </td>
            `;
    ...
    updateStatistics() {
        ...
        if (statsEl && this.checkRecords.length > 0) {
            ...
            statsEl.innerHTML = `
                ...
                <p><strong>🤖 识别模式：</strong>混合表格解析+AI增强</p>
            `;
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('🎀 婴幼儿健康追踪系统已初始化 (可爱卡通版 v10.0)');
